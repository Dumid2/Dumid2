GOODProjects
DC Government Grant Draft
DC Commission on the Arts & Humanities: FY27 Field Trip Experiences (FTE) Grant

STATUS: Draft for Neo's go/no-go review. Not yet submitted. This proposes a NEW field trip GOODProjects has not yet run. Stephanie Page-Baxter or Phileke Holland should confirm this is deliverable before it goes out.

## SOLICITATION DETAILS

| Agency | DC Commission on the Arts and Humanities (CAH) |
|---|---|
| Program / RFA | FY27 Field Trip Experiences (FTE) Grant |
| Ask Amount | $4,500 |
| Deadline | Friday, July 24, 2026, 5:00 pm ET (confirmed via CAH's official Field Trip Experiences grant page at dcarts.dc.gov/grants/field-trip-experiences, which resolves the earlier July 24 vs. July 27 discrepancy in favor of July 24) |
| Submission Method | CAH's online grants portal per dcarts.dc.gov. Direct site access was not available from this environment to confirm the exact portal URL/login; confirmed via public search results rather than a direct page load, so re-check the live submission mechanics before the deadline. |
| Program Officer | Not listed in the dev bible entry and not surfaced in public search results. Contact CAH generally through dcarts.dc.gov before submission. |
| Go/No-Go Confirmed | Not yet. Routing to Neo. |

VERIFY BEFORE SUBMITTING: this proposes a field trip to Arena Stage that GOODProjects has not yet scheduled. Confirm with Stephanie Page-Baxter (Director of Youth Programming) and Phileke Holland (Afterschool Lead) that this is realistic to deliver in the FY27 program year before submitting. Also confirm whether any of GOODProjects' partner schools (Amidon-Bowen Elementary, Jefferson Middle) carry Title I status, since that determines eligibility for Arena Stage's $4/ticket D.C. Ticket Partnership rate referenced in the budget below.

## SCOPE OF WORK / PROJECT NARRATIVE

### Program Purpose and Goals

GOODProjects requests $4,500 to provide a GOODLearningHub arts and humanities field trip experience: a weekday student matinee at Arena Stage, a professional theater in Southwest DC, for youth enrolled in GOODLearningHub and GOODCamp. This directly matches CAH's Field Trip Experiences goal of providing comprehensive arts and humanities field trip experiences that advance student achievement, paired with pre- and post-trip workshops and professional development for staff.

### Target Population

Residents of James Creek, Syphax Gardens, and Greenleaf public housing communities in Southwest DC's Ward 6, enrolled in GOODLearningHub.

| Total Unduplicated Participants | 30 youth per matinee session (Arena Stage's D.C. Ticket Partnership group size), drawn from GOODLearningHub's 73 enrolled students |
|---|---|
| Age Range | 5 to 14 |
| Primary Demographic | Black / African American (approximately 95%) |
| Income Level | Below 200% federal poverty line |
| Geographic Area | Ward 6, Southwest DC; field trip destination is Arena Stage, 1101 6th Street SW, Washington, DC 20024, itself in Southwest DC near GOODProjects' own service area |

### Activities and Deliverables

| Deliverable | Timeline |
|---|---|
| Select production and book a weekday student matinee through Arena Stage's D.C. Ticket Partnership or Group Sales | Month 1 |
| Pre-trip in-class workshop connecting the production's themes to GOODLearningHub's Social-Emotional Learning curriculum | Month 2, before the matinee |
| Field trip: matinee performance for 30 youth, including transportation | Month 2 |
| Post-show talkback with Arena Stage teaching artists | Same day as matinee |
| Professional development session for Youth Services Advocates on integrating theater-based learning into afterschool programming | Month 2 or 3 |
| Final Report | 30 days post-program |

### Staffing Plan

| Name | Role |
|---|---|
| Darius Baxter | Founder & CEO / Authorized Signatory |
| Demetruis Baxter | Chief of Staff |
| Stephanie Page-Baxter | Director of Youth Programming |
| Neo Mangope | Fundraising & Grants Officer |
| Phileke Holland | Afterschool Lead / Youth Program Coordinator (would lead trip logistics) |
| Cole Bradshaw | Finance & Operations |
| Melissa | Reporting & Stewardship |

### Evaluation and Performance Metrics

| Performance Measure | Target |
|---|---|
| Youth attending the matinee and both workshops | 30 (100% of the enrolled cohort) |
| Youth able to connect the production's themes to a personal or SEL discussion topic (post-trip survey) | 75% |
| Youth Services Advocates completing the professional development session | 100% of assigned staff |
| Reporting Frequency | Final report only, or per CAH's specific requirement once confirmed |

## BUDGET DETAIL

| Line Item | Basis / Calculation | Grant $ | Match $ |
|---|---|---|---|
| Tickets | 30 students + chaperones at $4/ticket (Arena Stage D.C. Ticket Partnership, Title I rate) | $150 | N/A |
| Transportation | Round-trip van/bus rental for 30 youth plus chaperones | $1,500 | N/A |
| Pre/post-trip workshop materials | Discussion guides, activity supplies | $500 | N/A |
| Staff planning and coordination time | Phileke Holland, partial time, trip planning and logistics | $1,500 | N/A |
| Contingency (printing, incidentals) | | $850 | N/A |
| TOTAL | | $4,500 | N/A |

VERIFY BEFORE SUBMITTING: this budget is built from typical GOODLearningHub transportation and staffing rates already documented elsewhere (GOODSports program budget), not from a confirmed CAH award cap or budget template. Confirm the actual FTE grant budget form and any per-grant maximum on dcarts.dc.gov before finalizing these figures.

## REQUIRED ATTACHMENTS CHECKLIST

[x] 501(c)(3) Determination Letter: on file
[x] Certificate of Good Standing (DC): on file
[ ] Business License: NOT ON FILE, same gap flagged on the Events DC draft
[x] Clean Hands Certificate: on file, issued 4/23/2026; reconfirm currency at submission
[x] Most recent audited financial statements (FY2024, Desai Company): on file
[x] Board of Directors list with affiliations: available
[ ] Agency-specific attachment (e.g., letter of commitment from Arena Stage, if CAH requires one): not yet obtained

Business License note: DC's Basic Business License (charitable solicitation subtype, DLCP) is required for DC government grant applications regardless of the $25,000 receipts exemption threshold. Apply through DLCP's online licensing wizard at mybusiness.dc.gov, which auto-generates the required attachment list. This is an operational task for Cole, not something a draft can resolve; start it now given the July 24 deadline.

## PRE-SUBMISSION QA GATE

- **01.** [x] All placeholders filled with real, verified figures
- **02.** [ ] Funder ask is within the stated cap: no published per-grant cap found; this is a modest, conservatively built estimate, not a confirmed ceiling
- **03.** [x] Budget total matches the headline ask
- **04.** [ ] Field trip logistics not yet confirmed with Stephanie Page-Baxter or Phileke Holland. Do this before submitting, not after; this is the one blocker only program staff can clear
- **05.** [x] No Cara Adams in the staffing section
- **06.** [x] Address reads: 996 Maine Avenue SW, Suite 208, Washington, DC 20024 (add to signature block before submission)
- **07.** [x] Geography confirmed: Ward 6 origin, DC-based destination (Arena Stage, Southwest DC)
- **08.** [x] Executive summary tailored to CAH's Field Trip Experiences priority
- **09.** [x] Proof metrics chosen match the funder's domain
- **10.** [x] Deadline confirmed: Friday, July 24, 2026, 5:00 pm ET (CAH's official grant page)
- **11.** [ ] Attachments not yet assembled. Business License still needs to be obtained through DLCP (see note above)
- **12.** [ ] Darius Baxter has not yet signed; route through Neo first
- **13.** [x] Humanize pass completed (no em or en dashes, no significance inflation, no vague attributions)
- **14.** [x] PDF generated: `2026-07-dc-arts-humanities-field-trip-arena-stage.pdf`

---

GOODProjects, Inc. | 996 Maine Avenue SW, Suite 208, Washington, DC 20024 | EIN 81-1491594 | grants@goodprojects.org
