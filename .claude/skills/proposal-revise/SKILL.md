...
description: Apply reviewer, collaborator, or self-review comments to an existing proposal draft while maintaining voice consistency. Use when applying feedback to a proposal draft.
---

> **Vendored from [chrisblattman/claudeblattman](https://github.com/chrisblattman/claudeblattman) (MIT).** Written for the author's own folder conventions (`05_Submissions/Grants/`, `~/.claude-assistant/donors/`) and Google Workspace MCP integration — none of which exist in this repo yet. Treat the workflow below as the reference design; paths need to be re-pointed at this repo's actual structure (`app/drafts/`, etc.) before relying on the automated steps. Flagged for review.
