---
name: proposal-write
description: Draft a funding proposal from structured inputs, using a project's voice pack, project context, and donor intelligence. Use when drafting funding proposals, LOIs, or project summaries for grant applications.
---

> **Vendored from [chrisblattman/claudeblattman](https://github.com/chrisblattman/claudeblattman) (MIT).** Written for the author's own folder conventions (`05_Submissions/Grants/`, `~/.claude-assistant/donors/`) and integrations (Google Workspace MCP, Granola, Gmail search) — none of which exist in this repo yet. Treat the workflow and checklist below as the reference design; paths and integrations need to be re-pointed at this repo's actual structure (`app/drafts/`, `app/org_profile/`, etc.) before relying on the automated steps. Flagged for review.
